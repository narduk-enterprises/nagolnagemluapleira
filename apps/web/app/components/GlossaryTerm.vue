...
<template>
  <UPopover :content="{ align: 'center', side: 'top' }">
    <UButton
      variant="link"
      color="neutral"
      class="p-0 underline decoration-[var(--color-primary)] decoration-2 underline-offset-4 cursor-help hover:decoration-[var(--color-accent)] transition-colors"
    >
      <slot>{{ term }}</slot>
    </UButton>
    ...
    <template #content>
      <div
        v-if="dictionaryEntry"
        class="w-80 p-4 relative no-print bg-[var(--color-card)] border border-[var(--color-border)] rounded-md shadow-md text-[var(--color-foreground)]"
      >
        <div class="space-y-2">
          <div class="flex items-start gap-2">
            <UIcon
              name="i-lucide-info"
              class="w-5 h-5 text-[var(--color-primary)] shrink-0 mt-0.5"
            />
            <div class="space-y-1">
